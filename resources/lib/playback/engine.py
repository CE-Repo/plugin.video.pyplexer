...
from core.common import is_resuming_video
from core.constants import CONFIG
from core.constants import StreamControl
...
from core.utils import write_pickled
...
def _to_seconds(milliseconds, default=0):
    """Plex reports times in milliseconds, Kodi wants whole seconds."""
    try:
...
def play_library_media(context, data):
    ...
    transcode = is_transcode_required(context, stream.get('details'), data.get('transcode', False))
    try:
    ...
    details = {
        'resume': _to_seconds(stream_media.get('viewOffset')),
        'duration': _to_seconds(stream_media.get('duration')),
    }

    if isinstance(data.get('force'), int):
        details['resume'] = _to_seconds(data['force']) if data['force'] > 0 else data['force']
...
def set_now_playing_properties(server, media_id):
    ...
    window.setProperty('plugin.video.pyplexer-nowplaying.server', server.get_location())
    window.setProperty('plugin.video.pyplexer-nowplaying.id', media_id)
...
class StreamData:
    ...
    def _get_video_data(self):
        self.data['full_data'] = {
            'plot': self._content.get('summary', ''),
            'title': self._content.get('title', i18n('Unknown')),
            'sorttitle': self._content.get('titleSort',
                                           self._content.get('title', i18n('Unknown'))),
            'rating': float(self._content.get('rating', 0)),
            'studio': [self._content.get('studio', '')],
            'mpaa': self._content.get('contentRating', ''),
        ...
        if self._content.get('type') == 'movie':
            ...
            self.data['full_data']['tvshowtitle'] = \
        ...
        if not self.context.settings.skip_metadata():
            ...
            if tree_genres is not None:
                self.data['full_data']['genre'] = \
                    list(map(lambda x: x.get('tag', ''), tree_genres))
    ...
    def _get_track_data(self):
        ...
        self.data['full_data'] = {
            ...
            'title': track_title,
            'rating': float(self._content.get('rating', 0)),
            'album': self._content.get('parentTitle', self.tree.get('parentTitle', '')),
            'artist': self._content.get('grandparentTitle',
...
class MediaSelect:
    ...
    def _select_media(self):
        ...
        if count > 1:
            ...
            if result == -1:  # dialog cancelled, nothing to play
                LOG.debug('Media selection cancelled')
                self._media_index = None
                return
            ...
            # options[0] is a (key, file) tuple; the original ``'.ifo' in
            # options[0]`` tested tuple membership and so never matched
            if force_dvd and any('.ifo' in (part or '').lower() for part in options[0]):
                self.dvd_playback = True
