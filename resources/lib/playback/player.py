...
from core.utils import read_pickled
from gui.dialogs.skip_intro import SkipIntroDialog
from playback.up_next import UpNext
...
class PlaybackMonitorThread(threading.Thread):
    ...
    def resume(self, current_time):
        ...
        # seek to resume time, it is greater than the current time
    ...
    def run(self):
        ...
        while self.PLAYER.isPlaying() and not self.MONITOR.abortRequested():
            ...
            if waited >= 10:  # only report every ~10 seconds, times are updated at 0.5 seconds
                waited = 0.0
    ...
    if control == StreamControl.PLEX:
        ...
        if stream['audio_count'] == 1:
            ...
            LOG.debug(
                ...
                audio.get('language', audio.get('languageCode', i18n('Unknown')))
            )
