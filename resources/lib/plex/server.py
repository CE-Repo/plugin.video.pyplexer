...
class PlexMediaServer:  # pylint: disable=too-many-public-methods, too-many-instance-attributes
    ...
    def __init__(self, server_uuid=None, name=None, address=None, port=32400,  # pylint: disable=too-many-positional-arguments
    ...
    def refresh(self):
        ...
        if (tree is not None and
            ...
            self.server_name = tree.get('friendlyName')
    ...
    def raw_xml(self, url):
        ...
        LOG.debug('PROCESSING: it took %.2f seconds to process data from %s' %
                  ((time.time() - start_time), self.get_address()))
        # data is the raw document, not an Element; ETree.tostring() used to be
        # called on it here and raised AttributeError on every call
        LOG.debugplus('RAW: %s' % data)
        return data
